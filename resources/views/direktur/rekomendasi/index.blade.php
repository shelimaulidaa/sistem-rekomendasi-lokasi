    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6 mb-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between mb-6 space-y-4 md:space-y-0">
            <form method="GET" action="{{ route('direktur.rekomendasi.index') }}" class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
                ...
                <select name="periode_id" class="w-full sm:w-auto min-h-[44px] border-gray-300 focus:border-primary focus:ring-primary rounded-md shadow-sm text-sm">
                    <option value="">Semua Periode</option>
                    @foreach($periodes as $periode)
                        <option value="{{ $periode->id }}" {{ $activePeriodeId == $periode->id || request('periode_id') == $periode->id ? 'selected' : '' }}>
                ...
                <div class="flex items-center gap-2">
                    ...
                    @if(request()->hasAny(['search', 'status', 'periode_id']) && (request('search') || request('status') || request('periode_id')))

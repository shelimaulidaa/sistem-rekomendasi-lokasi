<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between w-full">
            ...
            <a href="{{ route('direktur.observasi.export-pdf', $observasi->id) }}" class="inline-flex items-center justify-center px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-xs font-bold uppercase tracking-wider rounded-md shadow-sm transition-colors min-h-[38px]">
    ...
    <div class="max-w-6xl mx-auto py-6 px-4 sm:px-6 lg:px-8" x-data="{ activeTab: 'utama' }">
        ...
        <div x-show="activeTab === 'kelayakan'" class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-8" style="display: none;">
            ...
            <!-- SECTION 2: DATA PENILAIAN LOKASI -->
            <div class="pt-6 border-t border-gray-200">
                <div class="bg-gray-50/80 p-4 rounded-xl border border-gray-200/80 mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                    <div>
                        <div class="flex flex-wrap items-center gap-2">
                            <h3 class="text-lg font-bold text-base-dark">Data Penilaian Lokasi</h3>
                            <span class="px-2.5 py-0.5 text-xs font-bold bg-primary/10 text-primary rounded-full border border-primary/20">Hasil Penilaian</span>
                        </div>
                        <p class="text-xs text-gray-600 mt-1">Berikut merupakan hasil penilaian lokasi berdasarkan kriteria yang telah ditentukan.</p>
                    </div>
                </div>

                <!-- Group of 5 Criteria -->
                <div class="space-y-6">
                    <!-- Row 1: Biaya Sewa, Jarak RPH, Competitors (3 Summary Cards) -->
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <!-- Kriteria 1: Biaya Sewa -->
                        <div class="bg-white rounded-xl border border-gray-200 p-5 shadow-sm hover:border-primary/50 transition-colors">
                        ...
                        <!-- Kriteria 5: Tingkat Pesaing -->
                        <div class="bg-white rounded-xl border border-gray-200 p-5 shadow-sm hover:border-primary/50 transition-colors">
                            <div class="flex items-center justify-between border-b pb-3 mb-3">
                                <span class="text-xs font-bold text-gray-500 uppercase tracking-wider">5. Tingkat Pesaing</span>
                                <div class="p-2 bg-purple-50 text-purple-600 rounded-lg">
                                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
                                </div>
                            </div>
                            @php
                                $allCompetitors = $spatialData['competitors_list'] ?? [];
                                $compCount = (int) ($observasi->jumlah_kompetitor ?? count($allCompetitors));
                                $compList = array_slice($allCompetitors, 0, $compCount);
                            ...
                            <div class="flex items-baseline justify-between">
                                ...
                                @if($avgRating !== null && $avgRating > 0)
                                    <span class="text-xs font-bold text-yellow-600 bg-yellow-50 border border-yellow-200 rounded px-1.5 py-0.5">
                                        ★ {{ $avgRating }} / 5
                                    </span>
                                @endif
                            </div>
                            ...
                            @if(count($compList) > 0)
                                <div class="space-y-2 max-h-[120px] overflow-y-auto mt-2 pr-1">
                                    @foreach($compList as $comp)
                                        <div class="p-2 bg-gray-50 rounded border border-gray-100 text-xs">
                                            <div class="flex justify-between items-start">
                                                <span class="font-bold text-gray-800 text-[11px] truncate">{{ $comp['nama'] }}</span>
                                                <span class="text-[10px] text-gray-500 font-semibold bg-white border px-1 rounded flex-shrink-0 ml-1">{{ $comp['distance'] }} km</span>
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
                            @else
                                <p class="text-xs text-gray-400 italic mt-2">Tidak ada kompetitor terdekat.</p>
                            @endif
                        </div>

                    </div>

                    <!-- Row 2: Indikator Kelayakan Bangunan & Indikator Aksesibilitas Lokasi (Side-by-side Checklist Cards) -->
                        ...
                        <div class="bg-gray-50 rounded-xl p-5 border border-gray-200 shadow-sm">
                            ...
                            <ul class="space-y-3">
                                ...
                                <li class="flex items-center text-sm">
                                    ...
                                    <span class="{{ $observasi->bangunan_layak ? 'text-gray-800 font-semibold' : 'text-gray-400 line-through' }}">Kondisi bangunan baik (siap digunakan)</span>
                                </li>
                                <li class="flex items-center text-sm">
